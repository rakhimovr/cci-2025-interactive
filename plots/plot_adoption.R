# Plot: Reported Adoption
# Lines 994-1086 from cci-25.Rmd

create_adoption_plot <- function(df_summary, custom_colors) {
  
  # Reported adoption
  measures_ordered <- c("Reported adoption")
  plot_adoption <- lapply(1:length(measures_ordered), function(i) {
    ...
    custom_nudges <- data.frame(
      ...
      nudge_x = c(0, 0, 0, 0, 0.3, 0.3, 0),
      nudge_y = c(5, 5.5, -5.5, 5, 0, -6, -2.8)
    )
    ...
    p <- p + labs(y = "") +
      ...
      ggtitle(paste("U.S adults who say they have adopted climate actions")) + 
