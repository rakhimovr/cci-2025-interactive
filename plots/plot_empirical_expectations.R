# Plot: Empirical Expectations
# Lines 1185-1279 from cci-25.Rmd

create_empirical_expectations_plot <- function(df_summary, custom_colors) {
  
  # Norms
  # ## EE (Empirical expectations)
  measures_ordered <- c("Empirical expectations")
  
  plot_ee <- lapply(1:length(measures_ordered), function(i) {
    ...
    custom_nudges <- data.frame(
      ...
      nudge_x = c(0, 0, -0.5, -0.5, -0.5, 0.5, 0),
      nudge_y = c(5, 5, 0, 3, 3, -4, -4)
    )
    ...
    p <- p + labs(y = "") +
      ...
      ggtitle(paste("Perceived share of U.S. adults who has adopted climate actions")) + 
      guides(color = guide_legend(nrow = 2, byrow = TRUE, 
                                  keywidth = unit(0.8, "lines"),
                                  keyheight = unit(0.8, "lines"))) + 
      labs(subtitle = "% of U.S. adults believed to have adopted climate actions (empirical expectations)",
