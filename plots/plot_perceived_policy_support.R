# Plot: Perceived Policy Support
# Two-point trend analysis (2024-2025 only)

create_perceived_policy_support_plot <- function(df_summary, custom_colors) {
  
  # Perceived Policy Support
  measures_ordered <- c("Perceived policy support")
  
  plot_perceived_policy_support <- lapply(1:length(measures_ordered), function(i) {
    ...
    p <- p + labs(y = "") +
      ...
      ggtitle(paste("Perceived public support for climate policies")) + 
      guides(color = guide_legend(nrow = 2)) + 
      labs(subtitle = "% of U.S. adults believed to support policies to promote climate behaviors",
  ...
  if(is.null(plot_perceived_policy_support[[1]])) {
    return(NULL)
  } else {
    return(plot_perceived_policy_support[[1]])
  }
}