# Plot: Consideration
# Lines 1089-1177 from cci-25.Rmd

create_consideration_plot <- function(df_summary, custom_colors) {
  
  # Consideration
  measures_ordered <- c("Consideration")
  plot_consideration <- lapply(1:length(measures_ordered), function(i) {
    ...
    behavior_order <- c("Reduce food waste", "Install solar panels", "Drive an EV", 
                        "Eat less beef", "Install a heat pump", "Sign up for community solar", 
                        "Buy carbon offsets")
    
    custom_nudges <- data.frame(
      behavior_label = behavior_order,  # Changed from behavior to behavior_label
      nudge_x = c(0, 0, 0, 0, 0, 0.09, 0),
      nudge_y = c(5, 5.5, -5.5, -5.5, 5, -7.25, 4.5)
    )
    ...
    p <- p + labs(y = "") +
      ...
      scale_x_discrete(labels = x_labels) +  # Use dynamic labels
      ...
      ggtitle(paste("U.S. adults considering climate actions they haven't yet adopted")) + 
      guides(color = guide_legend(nrow = 2, byrow = TRUE, 
                                  keywidth = unit(0.8, "lines"),
                                  keyheight = unit(0.8, "lines"))) + 
      labs(subtitle = "% who have considered but not adopted each action. * Food waste question asked to everyone.",
           caption = paste0("Source: Climate Culture Index, Rare, ", max(unique_waves)))
