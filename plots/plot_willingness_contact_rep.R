# Plot: Willingness to Contact Representative
# Two-point trend analysis (2024-2025 only)

create_willingness_contact_rep_plot <- function(df_summary, custom_colors) {
  
  # Willingness to Contact Representative
  measures_ordered <- c("Willingness to contact representative")
  
  plot_contact_rep <- lapply(1:length(measures_ordered), function(i) {
    ...
    p <- p + labs(y = "") +
      ...
      ggtitle(paste("U.S. adult willingness to contact representatives about \nclimate policies")) + 
      guides(color = guide_legend(nrow = 2)) + 
      labs(subtitle = "% of U.S. adults willing to contact their representative to express support for climate policies",
  ...
  if(is.null(plot_contact_rep[[1]])) {
    return(NULL)
  } else {
    return(plot_contact_rep[[1]])
  }
}