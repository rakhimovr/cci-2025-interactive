# Plot: Personal Benefit
# Lines 1575-1668 from cci-25.Rmd

create_personal_benefit_plot <- function(df_summary, custom_colors) {
  
  ## Personal benefit
  measures_ordered <- c("Personal benefit")
  plot_personal_benefit <- lapply(1:length(measures_ordered), function(i) {
    ...
    custom_nudges <- data.frame(
      ...
      nudge_x = c(-0.3, -0.3, 0, -0.5, -0.3, 1, -0.3),
      nudge_y = c(3, 3, 3.5, 2, 2.75, -5.15, -4)
    )
    ...
    p <- p + labs(y = "") +
      ...
      ggtitle(paste("Average reported personal benefit from adopting climate actions")) + 
