# Plot: Perceived Difficulty
# Lines 860-954 from cci-25.Rmd

create_difficulty_plot <- function(df_summary, custom_colors) {
  
  # reported difficulty 
  measures_ordered <- c("Perceived difficulty")
  
  plot_difficulty <- lapply(1:length(measures_ordered), function(i) {
    ...
    custom_nudges <- data.frame(
      ...
      nudge_x = c(0, 0, 0, 0, -0.5, 0, 0),
      nudge_y = c(-8, 5, -5, -4.5, 8, 8, -5)
    )
    ...
    p <- p + labs(y = "") +
      ...
      ggtitle(paste("Average reported difficulty to engage in climate behaviors")) + 
