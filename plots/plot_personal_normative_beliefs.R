# Plot: Personal Normative Beliefs 
# Lines 1282-1376 from cci-25.Rmd

create_personal_normative_beliefs_plot <- function(df_summary, custom_colors) {
  
  ## PNB (Personal normative beliefs)
  measures_ordered <- c("Personal normative beliefs")
  
  plot_pnb <- lapply(1:length(measures_ordered), function(i) {
    ...
    custom_nudges <- data.frame(
      ...
      nudge_x = c(-0.5, -0.5, 1, -0.5, -0.4, 1, 0),
      nudge_y = c(4.5, 5, -5.5, 4.5, -5, -7.25, -4.75)
    )
    ...
    p <- p + labs(y = "") +
      ...
      ggtitle(paste("U.S. adults believing that adopting climate actions is the right \nthing to do")) + 
      guides(color = guide_legend(nrow = 2, byrow = TRUE, 
                                  keywidth = unit(0.8, "lines"),
                                  keyheight = unit(0.8, "lines"))) + 
      labs(subtitle = "Personal normative beliefs",
