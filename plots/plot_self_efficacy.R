# Plot: Self-efficacy
# Lines 1477-1570 from cci-25.Rmd

create_self_efficacy_plot <- function(df_summary, custom_colors) {
  
  measures_ordered <- c("Self-efficacy")
  plot_self_efficacy <- lapply(1:length(measures_ordered), function(i) {
    ...
    custom_nudges <- data.frame(
      ...
      nudge_x = c(0, 0, 0, 0, 0, 0.1, 0),
      nudge_y = c(4, 4, 4, 3.75, 3.5, 4, 4)
    )
    ...
    p <- p + labs(y = "") +
      ...
      ggtitle(paste("Average confidence in ability to adopt climate actions")) + 
      guides(color = guide_legend(nrow = 2, byrow = TRUE, 
                                  keywidth = unit(0.8, "lines"),
                                  keyheight = unit(0.8, "lines"))) + 
      labs(subtitle = "Self-efficacy",
