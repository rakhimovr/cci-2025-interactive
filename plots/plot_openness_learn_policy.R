# Plot: Openness to Learn About Policy
# Two-point trend analysis (2024-2025 only)

create_openness_learn_policy_plot <- function(df_summary, custom_colors) {
  
  # Openness to Learn About Policy
  measures_ordered <- c("Openness to learn about policy")
  
  plot_openness_learn <- lapply(1:length(measures_ordered), function(i) {
    ...
    p <- p + labs(y = "") +
      ...
      ggtitle(paste("U.S. adult openness to learn about climate policies")) + 
      guides(color = guide_legend(nrow = 2)) + 
      labs(subtitle = "% of U.S. adults who are open to learning more about policies to promote climate behaviors",
  ...
  if(is.null(plot_openness_learn[[1]])) {
    return(NULL)
  } else {
    return(plot_openness_learn[[1]])
  }
}