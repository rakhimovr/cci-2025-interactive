# Plot: Normative Expectations
# Lines 1379-1473 from cci-25.Rmd

create_normative_expectations_plot <- function(df_summary, custom_colors) {
  
  ## NE (Normative expectations)
  measures_ordered <- c("Normative expectations")
  
  plot_ne <- lapply(1:length(measures_ordered), function(i) {
    ...
    custom_nudges <- data.frame(
      behavior_label = behavior_order,  # Changed to behavior_label
      nudge_x = c(0, 0, -0.5, -0.5, 1.25, 1, -0.25),
      nudge_y = c(4, 4, 0, -3.5, 1.3, -6, -4)
    )
    ...
    p <- ggplot(df_measure, aes(x = factor(wave), y = mean, 
      ...
      geom_hline(yintercept = c(40, 60, 80), 
                 linetype = "dashed", color = "grey70", alpha = 0.2) +
    ...
    p <- p + labs(y = "") +
      ...
      ggtitle(paste("Perceived share of U.S. adults believing adopting climate actions is \nthe right thing to do")) + 
      guides(color = guide_legend(nrow = 2, byrow = TRUE, 
                                  keywidth = unit(0.8, "lines"),
                                  keyheight = unit(0.8, "lines"))) + 
      labs(subtitle = "Normative expectations",
