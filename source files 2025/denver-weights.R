...
hispanic <- read_csv("weights/Denver/hispanic-denver.csv")

# Hispanic
hispanic %<>% mutate(Freq = nrow(data_2025) * prop)
hispanic %<>% select(HISPANIC, Freq)

# Demo
age_sex_race <- read_csv("~/GitHub/cci-2025/Weights/Denver/age-gender-race-denver.csv")
