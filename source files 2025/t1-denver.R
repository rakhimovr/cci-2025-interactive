data_2025 %<>% mutate(hp_adoption = as.numeric(hp_adoption),
...
index_2025 <- index_2025 %>%
  select(RID, wt,  ev_ne:fw_policy.contact)
...
fin_table <- index_2025 %>% 
  mutate(behavior = sapply(strsplit(index_2025$indicator, split = '_', fixed = TRUE),function(x) (x[1])))
