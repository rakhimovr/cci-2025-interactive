...
# columns to drop: contain "national" or "boston"
drop_cols <- grep("national|boston", colnames(data_2025), value = TRUE)

# drop them from data_2025
data_2025 <- data_2025[ , !(names(data_2025) %in% drop_cols)]

data_2025 <- data_2025 %>% select(-Vrid, -Vcomment, -Vlanguage, -Vreferer, -Vsessionid, -Vuseragent)

data_2025 <- data_2025 %>% select(-ev_ee, -solar_ee)

colnames(data_2025) <- gsub("_denver$", "", colnames(data_2025))

# Lucid data cleaning

lucid_2025 <- read_csv("C:/Users/RakhimRakhimov/Documents/GitHub/cci-2025/data/denver_lucid.csv")
